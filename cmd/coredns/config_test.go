...
func TestConfigDefaults(t *testing.T) {
	...
	assert.Equal(t, "/var/lib/coredns-incus", cfg.DataDir)
...
func TestConfigFromEnvironment(t *testing.T) {
	t.Setenv("COREDNS_INCUS", "https://env:8443")
	t.Setenv("COREDNS_LISTEN", "127.0.0.1:5353")
	t.Setenv("COREDNS_WORKERS", "8")
	t.Setenv("COREDNS_DEBUG", "true")
...
func TestParseMarker(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			in:    "user.label.coredns.scope=global",
			key:   "user.label.coredns.scope",
			value: "global",
		},
		{
