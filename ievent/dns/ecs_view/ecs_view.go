...
type ECSView struct {
	...
	// AllowTransfer is who may ask for a zone transfer, set once before anything
	// serves. Empty allows nobody; the zone's own opt-in is the other half, both required.
...
func (v *ECSView) Replace(snap *Snapshot) {
	...
	if v.Metrics {
		zonesGauge.Set(float64(len(snap.ByZone)))
		addressesGauge.Set(float64(len(snap.ByAddr)))
	}
}
