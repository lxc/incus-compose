...
test-local folder="./..." *args:
    env -u INCUS_COMPOSE_URL -u INCUS_COMPOSE_CERT -u INCUS_COMPOSE_KEY -u INCUS_REMOTE -u INCUS_PROJECT go test {{ folder }} -v -coverprofile=coverage.out -covermode=atomic {{ args }}
...
# Dev install creates your dev environment: `just dev-install [container] [listen] [image]`
dev-install container_name="local:incus-compose-test" listen='127.0.0.1:1443' image='images:debian/trixie':
    @just make-nested "{{ container_name }}" "{{ image }}" "{{ listen }}"

# Run commands in the nested incus.
incus *args:
    @echo "Using remote '${INCUS_REMOTE-"${INCUS_COMPOSE_URL-"local"}"}':"
    incus {{ args }}
...
[private]
make-nested container='local:incus-compose-test' image='images:debian/trixie' listen="":
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    ./scripts/setup-nested-incus.sh -c "${cert_file}" -n "${container}" -i "${image}" -r stable -l "${listen}"
    set -e
