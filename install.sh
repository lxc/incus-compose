...
# Adapted from the golangci-lint install script
# (https://github.com/golangci/golangci-lint), which is based on the GoReleaser
# install-script template (https://goreleaser.com) and embeds the public-domain
# shell helpers from https://github.com/client9/shlib.
...
get_binaries() {
  case "$PLATFORM" in
    darwin/amd64) BINARIES="incus-compose" ;;
    darwin/arm64) BINARIES="incus-compose" ;;
    linux/amd64) BINARIES="incus-compose" ;;
    linux/arm64) BINARIES="incus-compose" ;;
    *)
